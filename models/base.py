...
class SirenLayer(nn.Linear):
    def __init__(self, input_dim, out_dim, bias=True, is_first=False,  w0=30):
...
class MLP(nn.Module):
    def __init__(self,
        ...
        input_ch_pts = input_chanel
        self.skips = skips
        self.D = D
        self.W = W
        self.embed_fn, input_ch_pts = get_embedder(embed_N_freqs, input_ch_pts)
        self.use_feature = use_geo_feature
        
        if self.use_feature:
            in_dim_0 = input_ch_pts + W_geo_feat
        else:
            in_dim_0 = input_ch_pts
        ...
        for l in range(D + 1):
            ...
            if l != D:
                ...
                if out_acti=='Sigmoid' or out_acti=='sigmoid':
                    layer = DenseLayer(in_dim, out_dim, activation=nn.Sigmoid())
                elif out_acti=='softplus' or out_acti=='SoftPlus':
...
if __name__ == "__main__":
    ...
    # test()
    def test2():
        albedo = MLP(out_acti='sigmoid')
        brdf = MLP(out_acti=None)
        print(albedo,'\n',brdf)
        multires=3
        embed_kwargs = {
        "include_input": True,  # needs to be True for ray_bending to work properly
        ...
        input = torch.tensor([[0.4074, 0.7190, 0.1248, 9.83737],
        [0.4684, 0.3474, 0.2266, 29038]])
