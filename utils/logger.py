...
class Logger(object):
    ...
    def add_imgs(self, imgs, class_name, it=-1, metric=None, batch_idx=None):
        if self.is_train:
            outdir = os.path.join(self.img_dir, 'epoch{:09d}'.format(it), 'batch{:09d}'.format(batch_idx.item()))
        else:
            outdir = os.path.join(self.img_dir, 'batch{:09d}'.format(batch_idx.item()))
        if not os.path.exists(outdir):
            os.makedirs(outdir)
        if self.multi_process_logging:
            dist.barrier()
        outfile = os.path.join(outdir, '{}.png'.format(class_name))
        if metric is not None:
            outfile = os.path.join(outdir, '{}_{:.2f}.png'.format(class_name, metric))
