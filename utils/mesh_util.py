...
def convert_sigma_samples_to_colored_ply(
    input_3d_sigma_array: np.ndarray,
    color_generator,
    voxel_grid_origin,
    volume_size,
    ...
    def batchify(query_fn, inputs: torch.Tensor, chunk=4096):
        out = []
        for i in tqdm(range(0, inputs.shape[0], chunk), disable=not True):
            out_i = query_fn(torch.from_numpy(inputs[i:i+chunk]).float().cuda()).data.cpu().numpy()
            out.append(out_i)
        out = np.concatenate(out, axis=0)
        return out
        
    color = batchify(color_generator.forward, mesh_points)
    color = color ** (1 / 2.2)
    ...
    for i in range(0, num_verts):
        verts_tuple[i] = tuple(np.concatenate([mesh_points[i, :], color[i, :]], axis=-1))
...
def extract_colored_mesh(implicit_surface, color_generator, volume_size=2.0, level=0.0, N=512, filepath='./surface.ply', show_progress=True, chunk=16*1024):
    s = volume_size
    voxel_grid_origin = [-s/2., -s/2., -s/2.]
    volume_size = [s, s, s]
    ...
    out = batchify(implicit_surface.forward, xyz)
    out = out.reshape([N, N, N])
    convert_sigma_samples_to_colored_ply(out, color_generator, voxel_grid_origin, [float(v) / N for v in volume_size], filepath, level=level)
