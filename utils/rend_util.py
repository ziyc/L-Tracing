...
def nerfactor_lift(x, y, z, intrinsics):
    device = x.device
    intrinsics = intrinsics.to(device)
    fx = intrinsics[..., 0, 0]
    fy = intrinsics[..., 1, 1]
    ...
    x_lift = (x - cx.unsqueeze(-1)) / fx.unsqueeze(-1)
    y_lift = -(y - cy.unsqueeze(-1)) / fy.unsqueeze(-1)
    
    return torch.stack((x_lift, y_lift, z), dim=-1)

def get_rays(datatype, c2w, intrinsics, H, W, N_rays=-1):
    device = c2w.device
    ...
    if datatype=='nerf':
        rays_d = nerfactor_lift(i, j, -1 * torch.ones_like(i).to(device), intrinsics)
        rays_d = (rays_d[:, :, None, :] * c2w[0, :3, :3]).sum(-1)
...
def sample_coarse(t_min: Union[torch.Tensor,float], t_max: Union[torch.Tensor,float], 
                  samples: int, det=False, device=None, prefix_shape=None,):
    if isinstance(t_min, torch.Tensor):
        device = t_min.device if device is None else device
        prefix_shape = t_min.shape if prefix_shape is None else prefix_shape
    else:
        assert prefix_shape is not None and device is not None, 'Please provide prefix_shape and device when given float type t_min&t_max'
        t_min = torch.ones(prefix_shape, device=device) * t_min
        t_max = torch.ones(prefix_shape, device=device) * t_max
    if det:
        t = torch.linspace(0.0, 1.0, steps=samples, device=device).repeat([*prefix_shape, 1])
        t_samples = t_min.unsqueeze(-1) * (1-t) + t_max.unsqueeze(-1) * t
    else:
        t = torch.linspace(0.0, 1.0, steps=(samples+1), device=device)
        t_samples = t_min.unsqueeze(-1) * (1-t) + t_max.unsqueeze(-1) * t
        _lower = t_samples[..., :-1]
        _upper = t_samples[..., 1:]
        t_rand = torch.rand([*prefix_shape, samples], device=device)
        t_samples = _lower + (_upper - _lower) * t_rand
    return t_samples
